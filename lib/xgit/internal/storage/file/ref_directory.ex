defmodule Xgit.Internal.Storage.File.RefDirectory do
  ...
  def init(git_dir) do
    ...
    {:ok,
     %{
       git_dir: git_dir,
       refs_dir: Path.join(git_dir, Constants.r_refs()),
       logs_dir: logs_dir,
       logs_refs_dir: Path.join(logs_dir, Constants.r_refs()),
       packed_refs_file: Path.join(git_dir, Constants.packed_refs())
     }}
  end
  ...
  def handle_create(%{refs_dir: refs_dir} = state) do
    ...
    File.mkdir_p!(Path.join(refs_dir, "tags"))
